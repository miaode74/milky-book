...
传统池化是不重叠的（stride = kernel size）。AlexNet 使用 stride ，kernel size （），即池化窗口有重叠 。这使得 top-1 和 top-5 错误率分别降低了 0.4% 和 0.3%，且更难过拟合 。
...
* 
**几何变换**：从  图像中随机裁剪  patches，并进行水平翻转 。这使训练集扩大了 2048 倍 。
