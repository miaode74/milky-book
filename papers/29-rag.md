...
3. **RAG 的工作流**：
* **Retriever**：根据输入  从 Wikipedia 索引中检索 Top-K 文档 。
* **Generator**：将输入  和检索到的文档  一起输入到 BART，生成输出 。
* 
...
论文提出了两种对潜变量  进行边缘化的方式：

**1. RAG-Sequence Model**
假设生成整个序列  时只使用同一个文档 。模型计算每个文档生成完整序列的概率，然后根据检索概率加权求和。
**公式**：
...
**2. RAG-Token Model**
允许在生成每个 token  时参考不同的文档。这使得模型可以综合多个文档的信息。
**公式**：
