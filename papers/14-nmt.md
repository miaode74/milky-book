...
### 核心问题
[cite_start]在本文发表之前（2014年），主流的神经机器翻译（NMT）模型大多属于 Encoder-Decoder 家族。这些模型通常将源句子编码为一个**固定长度的向量（fixed-length vector）**，然后基于该向量解码出译文 [cite: 11]。
[cite_start]作者通过理论推测和实验证明，这种“定长向量”是一个巨大的性能瓶颈 [cite: 12][cite_start]。因为神经网络需要将源句子（无论长短）的所有必要信息压缩到一个向量中，这导致模型在处理长句子时性能急剧下降 [cite: 22, 24]。

### 主要贡献
1.  [cite_start]**提出“联合对齐与翻译”架构**：不同于将整个句子压缩为单一向量，作者提出了一种扩展架构，允许模型在解码过程中自动（软）搜索源句子中与预测当前目标词相关的部分 [cite: 12]。
2.  [cite_start]**注意力机制（Attention Mechanism）**：引入了一种机制，使得解码器在生成每个词时，基于上下文向量（Context Vector）进行预测，而该向量是源句子所有注释（annotations）的加权和 [cite: 101]。
3.  [cite_start]**性能提升**：在英语-法语翻译任务上，该方法在长句子上的表现显著优于传统的 Encoder-Decoder 模型，且整体性能媲美当时的 SOTA 短语统计机器翻译系统 [cite: 13, 37]。

## 3. Introduction: 论文的动机是什么？请仔细梳理整个故事逻辑

### 传统方法的局限
[cite_start]传统的统计机器翻译（SMT）由许多独立调节的小组件组成，而神经机器翻译（NMT）试图构建一个单一的大型神经网络来完成翻译 [cite: 17]。
当时的 NMT 主流做法（如 RNN Encoder-Decoder）包含两个组件：
1.  [cite_start]**Encoder**：读取源句子并将其编码为固定长度向量 [cite: 19]。
2.  [cite_start]**Decoder**：从该编码向量输出翻译 [cite: 20]。
[cite_start]整个系统被联合训练以最大化正确翻译的概率 [cite: 21]。

### 动机来源：定长向量的诅咒
[cite_start]作者指出，Encoder-Decoder 方法的一个潜在问题是神经网络需要将源句子的所有必要信息“压缩”到固定长度的向量中 [cite: 22][cite_start]。这使得神经网络难以处理长句子，特别是那些比训练语料中的句子更长的句子 [cite: 23][cite_start]。实验表明，随着句子长度增加，基本 Encoder-Decoder 的性能会迅速恶化 [cite: 24]。

### 破局思路：从“压缩”到“检索”
为了解决这个问题，作者引入了一种新模型：
* [cite_start]**不再压缩**：模型不再尝试将整个输入句子编码为单一的定长向量 [cite: 31]。
* [cite_start]**动态编码**：将输入句子编码为一个向量序列 [cite: 32]。
* [cite_start]**自适应解码**：在解码翻译时，模型会自适应地选择这些向量的一个子集（Soft-search） [cite: 32]。
[cite_start]这种方法将编码器从“必须把所有信息塞进一个向量”的负担中解放出来 [cite: 33][cite_start]，使得信息可以散布在整个注释序列中，并由解码器根据需要进行检索 [cite: 123]。
...
* 
**前向 RNN** ：顺序读取输入 ，得到前向隐状态序列  。


* 
**后向 RNN** ：逆序读取输入，得到后向隐状态序列  。


* **注释向量 **：将前向和后向状态拼接： 。
这样， 就包含了围绕第  个词的整个序列的摘要信息 。



### 4.3 解码器：注意力机制 (Attention Mechanism)

这是论文的核心创新。解码器在时刻  生成目标词  时，不再依赖单一的 ，而是依赖**特定的上下文向量 **。

#### 步骤 1：计算对齐分数 (Alignment Score)

模型计算解码器上一时刻的隐状态  与编码器每一个注释  之间的匹配度（能量 ）：


作者将对齐模型  参数化为一个前馈神经网络（Feedforward NN）：
...
将分数通过 Softmax 归一化，得到权重 ：


这反映了在生成目标词  时，源句子中第  个词的重要性（或者说“注意力”分配）。
...
解码器的隐状态  由上一时刻状态 、上一时刻输出  和当前上下文向量  共同决定：
